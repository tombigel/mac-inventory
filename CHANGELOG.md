...
- Added tracked `mac-setup.wizard.yml` for declarative wizard menu labels, ordering, visibility, and defaults.
...
- Changed the guided backup wizard to generate missing backup-folder user config by default and ask whether to create new, overwrite, or use existing config when one exists; restore now offers to use an existing config when found.
- Changed wizard menu configuration to be repo-owned through the tracked `mac-setup.wizard.yml` file instead of user-generated by the CLI.
- Changed wizard dry-run defaults so backup defaults to no and restore defaults to yes.

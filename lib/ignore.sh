...
mi_ignore_update_config() {
  ...
  if [ "$action" = "ignore" ]; then
    MI_IGNORE_REF="$ref" MI_IGNORE_NAME="$name" yq e '
      .version = (.version // 1) |
      .restore.ignored_apps = (((.restore.ignored_apps // []) | map(select(.ref != strenv(MI_IGNORE_REF)))) + [{"ref": strenv(MI_IGNORE_REF), "name": strenv(MI_IGNORE_NAME)}])
...
mi_ignore_apply_config_to_inventory() {
  ...
  [ -f "$MI_CONFIG" ] || return 0
  mi_has yq || return 0
  refs="$(yq e -r '.restore.ignored_apps[]?.ref // ""' "$MI_CONFIG" 2>/dev/null | sed '/^$/d' || true)"
  [ -n "$refs" ] || return 0
  stamp="$(mi_timestamp)"
...
mi_ignore_command() {
  ...
  if [ "$count" -eq 0 ]; then
    rm -f "$matches"
    mi_error "no app entry matched: $token"
    return 1
  fi
  if [ "$count" -gt 1 ]; then
    mi_error "multiple app entries matched: $token"
  ...
  if [ "$MI_DRY_RUN" = "true" ]; then
    ...
    mi_info "dry-run: would update ignored app rules in $MI_CONFIG"
    mi_info "dry-run: would regenerate backup-list and README when supported"
    return 0
  fi

  mi_ignore_write_inventory "$MI_INVENTORY" "$path_expr" "$ref" "$action" || return 1
